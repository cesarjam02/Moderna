...
export const PermisoCreatePage: FunctionalComponent<{ path?: string }> = () => {
  ...
  const handleSubmit = async (e: Event) => {
    e.preventDefault();
    if (!formData.fechaInicio || !formData.fechaCaducidad) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      ...
      <form onSubmit={handleSubmit}>
        {step === 1 && <Step1InfoGeneral formData={formData} setFormData={setFormData} />}
        {step === 2 && <Step2ATS formData={formData} setFormData={setFormData} />}

        <div className="flex justify-between items-center mt-8">
          <Button 
            type="button" 
            onClick={() => step === 1 ? route('/permisos') : setStep(1)} 
          ...
          {step === 1 && (
            <Button type="button" onClick={() => setStep(2)} className="bg-blue-600 text-white hover:bg-blue-500">
              Siguiente: ATS
            </Button>
          )}
          {step === 2 && (
            <Button type="submit" disabled={loading} className="bg-green-600 text-white hover:bg-green-500">
...
// --- Step 1 Component ---
const Step1InfoGeneral = ({ formData, setFormData }) => {
    ...
    <div className="space-y-6">
      <div className={formSectionClass}>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <label className="block"><span className={formLabelClass}>Fecha de Inicio</span>
            <Input name="fechaInicio" type="date" value={formData.fechaInicio} onInput={handleDateChange} />
          </label>
          <label className="block"><span className={formLabelClass}>Fecha de Caducidad (Máx. 7 días)</span>
            <Input name="fechaCaducidad" type="date" value={formData.fechaCaducidad} onInput={handleDateChange} />
          </label>
        </div>
        <label className="block"><span className={formLabelClass}>Descripción General del Trabajo</span>
          <Input name="descripcionGeneral" value={formData.descripcionGeneral} onInput={handleInput} />
        </label>
        <div className="grid md:grid-cols-3 gap-4">
          <label className="block"><span className={formLabelClass}>Departamento</span>
            <select name="departamento" value={formData.departamento} onInput={handleInput} className={formSelectClass}>
              <option value="">Seleccione...</option>
              {DEPARTAMENTOS_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
          </label>
          <label className="block"><span className={formLabelClass}>Área</span>
            <Input name="area" value={formData.area} onInput={handleInput} />
          </label>
          <label className="block"><span className={formLabelClass}>Equipo o Maquinaria</span>
            <Input name="maquinaria" value={formData.maquinaria} onInput={handleInput} />
          </label>
        </div>
      </div>
      
      <div className={formSectionClass}>
        <h3 className="text-lg font-semibold text-white">Tipos de Trabajo Involucrados</h3>
      ...
      <div className={formSectionClass}>
        <h3 className="text-lg font-semibold text-white">Personal Autorizado</h3>
...
// --- Step 2 Component ---
const Step2ATS = ({ formData, setFormData }) => {
  ...
  return (
    <div className="space-y-6">
      <div className={formSectionClass}>
        ...
        <label className="block"><span className={formLabelClass}>Cantidad de Personas Expuestas</span>
          <Input name="cantidadPersonas" type="number" value={formData.ats.cantidadPersonas} onInput={handleAtsInput} />
        </label>
      </div>
  ...
  return (
    <div className={`${formSectionClass} !p-4`}>
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <CheckboxList title="Peligros Identificados" items={LISTADO_PELIGROS} checkedItems={tarea.peligros} onChange={(e, item) => handleCheckbox(e, 'peligros', item)} />
        <CheckboxList title="Medidas de Control" items={LISTADO_MEDIDAS} checkedItems={tarea.medidas} onChange={(e, item) => handleCheckbox(e, 'medidas', item)} />
      </div>
    </div>
  );
};
